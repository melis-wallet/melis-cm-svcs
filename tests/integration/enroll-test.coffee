...
module('Integration: enroll',
  ...
  beforeEach: (assert)->
    ...
    creds = lookupService(application, 'cm-credentials')
...
test '[a] Normal enroll process (stateless)', (assert) ->
  ...
  .then( (wallet) ->
    ...
    assert.equal(Ember.isEmpty(session.get('accounts')), true, '[a]: Account is empty')
  ...
  .then( (res)->
    ...
    assert.equal(Ember.isBlank(session.get('currentWallet')), true, '[a] No wallet is open')
  )
...
test '[b] Normal enroll process (with state)', (assert) ->
  ...
  # check it closed
  .then( (res)->
    assert.equal(Ember.isBlank(session.get('currentWallet')), true, '[b] No current wallet')
...
test '[c] Credentials reset', (assert) ->
  ...
  .then( (res) ->
    ...
    assert.equal(Ember.isBlank(session.get('currentWallet')), true, '[c] No current wallet')
...
test '[d] Enroll, export, import', (assert) ->
  ...
  .then( (res)->
    ...
    assert.equal(Ember.isBlank(session.get('currentWallet')), true, '[d] No current wallet')
...
test '[e] Enroll, export, failed import', (assert) ->
  ...
  .then( (res)->
    ...
    assert.equal(Ember.isBlank(session.get('currentWallet')), true, '[e] No current wallet')
...
test '[f] Enroll, plaintex export, import', (assert) ->
  ...
  .then( (res)->
    ...
    assert.equal(Ember.isBlank(session.get('currentWallet')), true, '[f] No current wallet')
